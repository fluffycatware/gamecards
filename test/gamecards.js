const ETHCards = artifacts.require('./GameCards.sol')

contract('GameCards', (accounts) => {
  const owner = accounts[0]
  const withdrawWallet = accounts[1]
  const account1 = accounts[2]
  const account2 = accounts[3]
  const fluffyCat = '0xb39274C9887d314Ba65dA9929f3d0E94893570A7'
  ...
  it('should credit contract owner & fluffyCat on initial buy', function() {
    ...
    return ETHCards.new(owner)
      ...
      .then(function(res) {
        ...
        return APP.getBalance({from: fluffyCat })
      })
      .then(function(res) {
